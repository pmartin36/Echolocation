...
Shader "Universal Render Pipeline/Screen Space Decal/Echo"
{
    Properties
    {
        [Header(Basic)]
        ...
        _Radius("Max, Min, Offset", Vector) = (0,0,0)
        ...
        SubShader
        {
            ...
            float2 uv = decalSpaceUV.xy * _MainTex_ST.xy + _MainTex_ST.zw;//Texture tiling & offset
                ...
                float rmod = frac(length(uvm) - _Radius.z);
                //float rmod = frac(length(uvm) - _Radius.z);
                float f = 25 * pow(rmod, 2);
                float cosf = cos(f);
                
                // for waves that decrease in size as they get further from radius
                //float rmod = frac(length(uvm)); 
                //float f = 25 * pow(rmod, 2);
                //float cosf = cos(f + _Radius.z);
                
                cosf *= smoothstep(_Radius.x, _Radius.x+0.1, r);
                cosf *= smoothstep(_Radius.y, _Radius.y-0.1, r);
